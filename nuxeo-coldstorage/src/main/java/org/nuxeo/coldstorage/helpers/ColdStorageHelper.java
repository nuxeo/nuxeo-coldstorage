...
public class ColdStorageHelper {
    ...
    public static DocumentModel requestRetrievalFromColdStorage(CoreSession session, DocumentRef documentRef,
        ...
        if (Boolean.TRUE.equals(beingRetrieved)) {
            throw new NuxeoException(
                    String.format("The cold storage content associated with the document: " +
                            documentModel + " is being retrieved."),
                    SC_FORBIDDEN);
        }
    ...
    public Optional<DocumentModel> getDocumentModelById(CoreSession session, String id) {
        return Optional.of(session.getDocument(new IdRef(id)));
    }

    /**
     * Checks the {@link DocumentModel} of the given {@code id} allowed the given {@code transition}.
     *
     * @param session    the core session
     * @param documentId the document id
     * @param transition the transition to check
     * @return an optional containing the document model of the given {@code id} if it exists, {@code false} otherwise
     */

    public boolean isAllowedTransition(CoreSession session, String documentId, String transition) {
        DocumentModel documentModel = getDocumentModelById(session, documentId).get();
        for (String allowedTransition : documentModel.getAllowedStateTransitions()) {
            return allowedTransition.equals(transition);
        }
    ...
    public static class ColdStorageContentStatus {
        ...
        protected int totalBeingRetrieved = 0;
