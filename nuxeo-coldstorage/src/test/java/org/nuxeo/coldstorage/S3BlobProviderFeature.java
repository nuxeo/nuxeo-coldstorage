...
public class S3BlobProviderFeature implements RunnerFeature {
    ...
    public void start(FeaturesRunner runner) {
        ...
        String awsId = configureProperty(AWS_ID, sysEnv(ACCESS_KEY_ENV_VAR), sysEnv(ALTERNATE_ACCESS_KEY_ENV_VAR),
                sysProp(AWS_ID));
        String awsSecret = configureProperty(AWS_SECRET, sysEnv(SECRET_KEY_ENV_VAR), sysEnv(ALTERNATE_SECRET_KEY_ENV_VAR),
                sysProp(AWS_SECRET));
        ...
        String awsRegion = configureProperty(BUCKET_REGION, sysEnv(AWS_REGION_ENV_VAR), sysProp(BUCKET_REGION));
        // configure specific blob provider properties
        String testBucket = configureProperty(PROVIDER_TEST_BUCKET, sysProp(PROVIDER_TEST_BUCKET), sysProp(BUCKET));
        ...
        String otherBucket = configureProperty(PROVIDER_OTHER_BUCKET, sysProp(PROVIDER_OTHER_BUCKET),
        ...
        try {
            ...
            Bundle bundle = harness.getOSGiAdapter()
                                   .getRegistry()
                                   .getBundle("org.nuxeo.coldstorage.test");
