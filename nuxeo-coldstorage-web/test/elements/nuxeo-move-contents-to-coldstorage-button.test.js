    ...
    suite('Visibility', () => {
      test('Should not be visible when documents are not provided', async () => {
        button = await fixture(
          html` <nuxeo-move-contents-to-coldstorage-button></nuxeo-move-contents-to-coldstorage-button> `,
        );
      ...
      test('Should not visible when at least one of the documents is under retention or legal hold', async () => {
        documents[0].isUnderRetentionOrLegalHold = true;
        button = await fixture(
          html`
            <nuxeo-move-contents-to-coldstorage-button
              .documents=${documents}
