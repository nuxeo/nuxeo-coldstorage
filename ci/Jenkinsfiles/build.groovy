...
pipeline {
  ...
  stages {
            ...
            container('maven') {
              nxWithGitHubStatus(context: 'utests/backend/dev') {
                try {
                  ...
                  retry(3) {
                    sh 'mvn -B -nsu -pl :nuxeo-coldstorage -Dcustom.environment=dev test'
                  }
                } finally {
    ...
    stage('Run functional tests') {
        ...
        container('maven') {
          nxWithGitHubStatus(context: 'docker/build') {
            script {
              ...
              def nuxeoVersion = readMavenPom().getParent().getVersion()
      ...
      post {
        always {
          archiveArtifacts artifacts: 'nuxeo-coldstorage-web/target/screenshots/**', allowEmptyArchive: true
          cucumber(fileIncludePattern: '**/*.json', jsonReportDirectory: 'nuxeo-coldstorage-web/target/cucumber-reports/',
          ...
          nxWithGitHubStatus(context: 'package/deploy', message: 'Deploy Nuxeo packages') {
            script {
              ...
              nxUtils.postForm(credentialsId: 'connect-preprod', url: "${CONNECT_PREPROD_SITE_URL}marketplace/upload?batch=true",
                  form: ["package=@${NUXEO_COLDSTORAGE_PACKAGE_PATH}"])
            }
          }
        }
      }
